...
test('DESC partial — gaps filled with 0', () => {
  ...
  return TPS.sanitizeTimeInput(DESC_PARTIAL) === 'T:greg.m3.c1.y26.m01.d09.h14.m0.s25.m0';
});
...
test('calendar-only string — all 9 slots filled with 0', () => {
  return TPS.sanitizeTimeInput('T:greg') === 'T:greg.m0.c0.y0.m0.d0.h0.m0.s0.m0';
});

test('DESC with negative millennium — preserved', () => {
  const out = TPS.sanitizeTimeInput('T:greg.m-1.c9.y99.m12.d31.h23.m59.s59.m999');
...
test('ASC full — reversed to DESC', () => {
  ...
  return TPS.sanitizeTimeInput(ASC_FULL) === 'T:greg.m3.c1.y26.m1.d9.h14.m30.s25.m0';
});

test('ASC partial — reversed and gaps filled', () => {
  // s10.h08.d15.y25.c2 (asc) → reversed: c2.y25.d15.h08.s10, no m tokens → all m slots = 0
  return TPS.sanitizeTimeInput(ASC_PARTIAL) === 'T:greg.m0.c2.y25.m0.d15.h08.m0.s10.m0';
});

test('ASC URI @T: — reversed, prefix preserved', () => {
  return TPS.sanitizeTimeInput(URI_ASC) === 'tps://L:32.01,35.86@T:greg.m3.c1.y26.m1.d9.h14.m30.s25.m0';
});

test('ASC URI /T: — reversed, prefix preserved', () => {
  // `/T:` is normalised to `@T:` before reversal occurs
  return TPS.sanitizeTimeInput(URI_SLASH_ASC) === 'tps://L:32.01,35.86@T:greg.m0.c2.y25.m0.d15.h08.m0.s10.m0';
});
...
test('suffix not included in token reversal for ASC URI', () => {
  ...
  const out = TPS.sanitizeTimeInput('tps://unknown@T:greg.s25.h14.d9.y26.c1!sig1');
...
test('URI without T: section — false', () => !TPS.validate('tps://L:32.01,35.86'));
...
test('URI with unknown location but no T: — false', () => !TPS.validate('tps://unknown'));
...
test('DESC URI — location lat/lon captured', () => {
  ...
  return !!c && c.latitude === 32.01 && c.longitude === 35.86 && c.millennium === 3;
});
...
test('toURI emits ASC when order=ASC', () => {
  const comp: any = {
    ...
    millennium: 3, century: 1, year: 26,
    month: 1, day: 9, hour: 14, minute: 30, second: 25,
    order: TimeOrder.ASC,
  };
...
test('fromDate DESC — canonical format', () => {
  ...
  return typeof s === 'string' && s.includes('T:greg.m3.c1.y26.m1.d9.h14.m30.s25');
});
