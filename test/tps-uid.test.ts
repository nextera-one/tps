 ...
 * @version 1.0
 */
import { TPSUID7RB, TPSUID7RBDecodeResult } from "../src/index";
...
test("Missing prefix throws error", () => {
  try {
    ...
    return (e as Error).message.includes("missing prefix");
  }
});

test("Bad magic throws error", () => {
  try {
    // keep minimum valid structure length so decoder reaches magic check
    const badBytes = new Uint8Array(17);
    badBytes.set([0x00, 0x00, 0x00, 0x00, 0x01, 0x00], 0);
    TPSUID7RB.decodeBinary(badBytes);
    return false;
  } catch (e) {
    return (e as Error).message.includes("bad magic");
  }
});
...
test("Too short throws error", () => {
  try {
    ...
    return (e as Error).message.includes("too short");
  }
});
...
test("Unsupported version throws error", () => {
  try {
    ...
    return (e as Error).message.includes("unsupported version");
  }
});
...
test("Tampered sealed UID fails verification", () => {
  ...
  try {
    ...
    return (e as Error).message.includes("verification failed");
  }
});
...
test("Verify fails with wrong public key", () => {
  ...
  try {
    ...
    return (e as Error).message.includes("verification failed"); // Or openssl error
  }
});
